...
export default {
    ...
    create: async (name, description) => {
        ...
        return await db(TABLE_NAME.categories).insert({ name, description }).returning('id')
    ...
    update: async (id, name, description) => {
        ...
        const categories = await db(TABLE_NAME.categories).where({ name }).whereNot({ id })
        if (categories.length > 0) throw new Error("The category name already exists")

        return await db(TABLE_NAME.categories).where({ id }).update({ name, description })
    },
    delete: async id => {
